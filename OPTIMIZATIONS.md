...
- **正确性**：远端随机测试退出码 0，输出 `Correctness test passed!`；两次正式四 case 均通过。汇编确认 `zmm` 和 `_ZGVeN8v_exp@PLT`。
- **两侧原始数字**：最终两次时间分别为 `0.025536/0.033847/0.097961/0.042941 s` 和 `0.025860/0.035038/0.098592/0.042801 s`；speedup 为 `45.733/67.411/574.211/16.019x` 和 `45.160/65.120/570.536/16.071x`；加权 `143.878x / 142.591x`，中值 `143.235x`。
- **旁置条件**：包含 EXP-006 scatter 和 EXP-007 slab-BLAS；相对 EXP-007 的完整加权受 case 3 噪声混淆，case 4 从 `0.052778 s` 稳定降至约 `0.043 s` 是最清楚证据。
- **推理链**：自动向量化两次失败且汇编仍为标量 `exp`；显式 ABI 后 codegen 和 case 4 时间同时改变，机制成立。
- **Outcome**: keep；当前最优
